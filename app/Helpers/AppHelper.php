...
// Upload and show image in Aws3
function get_file_url_s3($file_id, $size = "thumb", $resize = true)
{
    if (empty($file_id)) return null;
    return \Modules\Media\Helpers\FileHelper::url_s3($file_id, $size, $resize);
}

function get_image_tag_s3($image_id, $size = 'thumb', $options = [])
{
    $options = array_merge($options, [
        'lazy' => true
    ]);

    $url = get_file_url_s3($image_id, $size);
    // dd($url);
    if ($url) {
        $alt = $options['alt'] ?? '';
        $attr = '';
...
// Show div with price instead of img
// 
