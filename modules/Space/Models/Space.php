...
class Space extends Bookable
{
    ...
    // Get thumb image from Aws3
    public function getGalleryS3($featuredIncluded = false)
    {
        ...
        if ($featuredIncluded and $this->image_id) {
            $list_item[] = [
                'large' => FileHelper::url_s3($this->image_id, 'full'),
                'thumb' => FileHelper::url_s3($this->image_id, 'thumb')
            ];
        }
        $items = explode(",", $this->gallery);
        foreach ($items as $k => $item) {
            $large = FileHelper::url_s3($item, 'full');
            $thumb = FileHelper::url_s3($item, 'thumb');
            $list_item[] = [
                'large' => $large,
                'thumb' => $thumb
            ];
    ...
    public static function search(Request $request)
    {
        ...
        $current_path = serverPath();
        if ($current_path) {
            $model_space->selectRaw('MAX(bravo_spaces.id) as max_id')->groupBy("bravo_spaces.contact")->orderBy('id', 'desc');
        } else {
