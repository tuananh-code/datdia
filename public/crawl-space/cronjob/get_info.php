...
function crawl_info($url, $page, $get_name, $get_address, $get_bed, $get_bath)
{
    // Initialize cURL
    $curl = curl_init();
    // Set cURL options
    ...
    processResponse_info($response, $url, $get_name, $get_address, $get_bed, $get_bath);
    // getAllPages($response);
}

function processResponse_info($response, $url, $get_name, $get_address, $get_bed, $get_bath)
{
    include '../config.php';
    $dom = new DOMDocument();
    ...
    if (strpos($url, 'century21')) {
        $info_name = str_replace("'", "''", $get_name);
        $slug = vn_to_str(mb_strtolower(trim(str_replace("'", "''", $info_name))));
        $convert_slug = str_replace('--', '-', $slug);
        $get_location = str_replace("'", "''", $get_address) . ', Australia';
        $get_bedroom = $get_bed;
        $get_bathroom = $get_bath;
        //price
