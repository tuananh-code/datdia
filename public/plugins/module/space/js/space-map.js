jQuery(function ($) {
    ...
    $(".bravo_form_search_map .g-map-place input[name=map_place]").change(
        function () {
            reloadForm();
        }
    );
    ...
    $(".bravo_form_search_map .btn-filter,.btn-apply-advances").click(
        function () {
            reloadForm();
        }
    );
    ...
    function reloadForm() {
        ...
        $.ajax({
            data:
                $(
                    ".bravo_form_search_map input,select,textarea,input:hidden,#advance_filters input,select,textarea"
                ).serialize() + "&_ajax=1",
    ...
    $(document)
        .on("click", ".filter-item .dropdown-menu", function (e) {
